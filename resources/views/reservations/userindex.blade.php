    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($reservations as $reservation)
                <a href="{{ route('events.show', $reservation->event->id) }}" class="block p-6 bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-200">
                    ...
                    <div class="text-sm text-gray-500 mb-2">
                        <span class="font-semibold">Fecha:</span> {{ \Carbon\Carbon::parse($reservation->event->date)->format('d-m-Y') }} |
                        <span class="font-semibold">Hora:</span> {{ \Carbon\Carbon::parse($reservation->event->time)->format('H:i') }}
                    </div>
                    <div class="text-sm text-gray-500 mb-2">
